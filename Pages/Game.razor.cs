...
public partial class Game : ComponentBase {
    ...
    private WebGLShader shadowShader;
    ...
    private int positionAttribLocationShadow;
    private int normalAttribLocation;
    private int normalAttribLocationShadow;
    private int colorAttribLocation;
    private int colorAttribLocationShadow;
    private WebGLUniformLocation projectionUniformLocation;
    private WebGLUniformLocation projectionUniformLocationShadow;
    private WebGLUniformLocation modelViewUniformLocation;
    private WebGLUniformLocation modelViewUniformLocationShadow;
    private WebGLUniformLocation normalTransformUniformLocation;
    private WebGLUniformLocation normalTransformUniformLocationShadow;
    ...
    private async Task getShadowAttributeLocations() {
        this.positionAttribLocationShadow = await this._context.GetAttribLocationAsync(this.shadowProgram,"aVertexPosition");
        this.normalAttribLocationShadow = await this._context.GetAttribLocationAsync(this.shadowProgram,"aVertexNormal");

        this.colorAttribLocationShadow = await this._context.GetAttribLocationAsync(this.shadowProgram,"aVertexColor");
        this.projectionUniformLocationShadow = await this._context.GetUniformLocationAsync(this.shadowProgram,"uProjectionMatrix");
        this.modelViewUniformLocationShadow = await this._context.GetUniformLocationAsync(this.shadowProgram,"uModelViewMatrix");
        this.normalTransformUniformLocationShadow = await this._context.GetUniformLocationAsync(this.shadowProgram,"uNormalTransformMatrix");
    }
    ...
    private void updatePawn(){
        ...
        if(displacement.Norm()>0){
            ...
            //List<string> collisionCollection;
            //collistionCollection=updatePawnCollision();
            if(checkActorCollision("apawn"))
                ActiveLevel.ActorCollection["apawn"].Transform.Copy(previousTransform);
        }
    }
    ...
    public async Task Draw(){
        ...
        await this._context.UniformMatrixAsync(this.projectionUniformLocationShadow,false,this.ProyMat.GetArray());
        ...
        foreach( var keyval in ActiveLevel.ActorCollection){
           ...
           foreach(var smv in actor.ModelViewShadow){
                await this.getShadowAttributeLocations();
                await this._context.UseProgramAsync(this.shadowProgram);

                await this._context.UniformMatrixAsync(this.modelViewUniformLocationShadow,false,actor.ModelView.GetArray());
                await this._context.UniformMatrixAsync(this.normalTransformUniformLocationShadow,false,actor.NormalTransform.GetArray());
                    
                await this._context.BindBufferAsync(BufferType.ARRAY_BUFFER, mBuffers.VertexBuffer);
                await this._context.EnableVertexAttribArrayAsync((uint)this.positionAttribLocationShadow);
                await this._context.VertexAttribPointerAsync((uint)this.positionAttribLocationShadow,3, DataType.FLOAT, false, 0, 0L);
            

                await this._context.BindBufferAsync(BufferType.ARRAY_BUFFER, mBuffers.NormalBuffer);
                await this._context.EnableVertexAttribArrayAsync((uint)this.normalAttribLocationShadow);
                await this._context.VertexAttribPointerAsync((uint)this.normalAttribLocationShadow,3, DataType.FLOAT, false, 0, 0L);



                await this._context.BindBufferAsync(BufferType.ARRAY_BUFFER, mBuffers.ColorBuffer);
                await this._context.EnableVertexAttribArrayAsync((uint)this.colorAttribLocationShadow);
                await this._context.VertexAttribPointerAsync((uint)this.colorAttribLocationShadow,4, DataType.FLOAT, false, 0, 0L);
                
                await this._context.UniformMatrixAsync(this.modelViewUniformLocationShadow,false, smv.GetArray());
                await this._context.DrawElementsAsync(Primitive.TRIANGLES,mBuffers.NumberOfIndices,DataType.UNSIGNED_SHORT, 0);
           }
...
// Resources
        ...
        dirLightDiffuseLocation = new WebGLUniformLocation[this.NumberOfDirectionalLights];
            ...
            this.shadowShader=await this.GetShader(shadowSource,ShaderType.FRAGMENT_SHADER);
            ...
            this.shadowProgram = await this.BuildProgram(this.vertexShader, this.shadowShader);
            
            await this._context.DeleteShaderAsync(this.vertexShader);
            await this._context.DeleteShaderAsync(this.shadowShader);
